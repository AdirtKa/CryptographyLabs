...
Реализация алгоритма DES с режимом CBC
"""
...
# ═════════════════════════════════════════════════════════════════════════════
# ФУНКЦИИ ШИФРОВАНИЯ CBC (Cipher Block Chaining)
# ═════════════════════════════════════════════════════════════════════════════
...
def encrypt_cbc(data: bytes, key: bytes, iv: bytes) -> bytes:
    ...
    2. Для каждого блока: XOR(блок, предыдущий_зашифрованный_блок)
    3. Шифруем результат XOR
    4. Результат становится предыдущим для следующего блока
...
def decrypt_cbc(data: bytes, key: bytes, iv: bytes) -> bytes:
    ...
    Процесс декодирования CBC:
    1. Разбиваем зашифрованные данные на блоки по 8 байт
    2. Для каждого блока: расшифровываем, XOR с предыдущим зашифрованным блоком
    3. Предыдущий зашифрованный блок становится IV для первого блока
...
def main() -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            encrypted_message = encrypt_cbc(message.encode('utf-8'), key, iv)
