...
def word_to_bits(data: bytes) -> list[int]:
    """Преобразует bytes в список битов."""
    return [int(b) for byte in data for b in f"{byte:08b}"]


def bits_to_bytes(bits: list[int]) -> bytes:
    """Обратно: список битов → bytes."""
...
def encrypt(plaintext: bytes, key: bytes) -> bytes:
    key_bits = word_to_bits(key)
    permuted_key_bits = permute_bits(key_bits, pc1)
    round_keys = get_round_keys(permuted_key_bits)

    word_bits = word_to_bits(plaintext)
    permuted_word_bits = permute_bits(word_bits, ip)

    left = permuted_word_bits[:32]
    right = permuted_word_bits[32:]

    for round_key in round_keys:
        new_left = make_round(left, right, round_key)
        left, right = right, new_left

    cipher_bits = permute_bits(right + left, ip_inverse)
...
def decrypt(ciphertext: bytes, key: bytes) -> bytes:
    ...
    word_bits = word_to_bits(ciphertext)
    permuted_word_bits = permute_bits(word_bits, ip)
    ...
    plain_bits = permute_bits(right + left, ip_inverse)
...
def encrypt_message(plaintext: str, key: str) -> str:
    """Шифрует сообщение (строка -> base64)."""
    data = pad(plaintext.encode("utf-8"))
    result = b""
    for i in range(0, len(data), 8):
        block = data[i:i + 8]
        result += encrypt(block, key.encode("utf-8"))
    return base64.b64encode(result).decode("utf-8")


def decrypt_message(ciphertext: str, key: str) -> str:
    """Расшифровывает сообщение (base64 -> строка)."""
    data = base64.b64decode(ciphertext.encode("utf-8"))
    result = b""
    for i in range(0, len(data), 8):
        block = data[i:i + 8]
        result += decrypt(block, key.encode("utf-8"))
